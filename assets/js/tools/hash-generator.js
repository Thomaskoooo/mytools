(function () {
  ...
  function md5(input) {
    ...
    function convertToWordArray(str) {
      var wordCount, messageLength = str.length;
      var t1 = messageLength + 8;
      var t2 = (t1 - (t1 % 64)) / 64;
      var numberOfWords = (t2 + 1) * 16;
      var wordArray = new Array(numberOfWords - 1).fill(0);
      var byteCount = 0, bytePosition;
    ...
    var x = convertToWordArray(utf8Encode(input));
  ...
  async function generate() {
    ...
    var resultsBox = document.getElementById('hash-results');
    if (!text) { resultsBox.style.display = 'none'; return; }

    var results = [
      ['MD5', md5(text), true],
      ['SHA-1', await sha('SHA-1', text), true],
      ['SHA-256', await sha('SHA-256', text), false],
      ['SHA-512', await sha('SHA-512', text), false]
    ];

    resultsBox.innerHTML = results.map(function (r) {
      var warn = r[2] ? ' <span style="color:var(--warning);font-weight:700">⚠ cryptographically broken</span>' : '';
      return '<div class="result-item" style="align-items:flex-start;flex-direction:column;gap:4px">' +
        '<span class="r-label">' + r[0] + warn + '</span>' +
        '<span style="display:flex;align-items:center;gap:6px;width:100%"><span class="r-value" style="word-break:break-all;flex:1">' + r[1] + '</span>' +
  ...
  document.addEventListener('DOMContentLoaded', function () {
    document.getElementById('hash-input').addEventListener('input', generate);
